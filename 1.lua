-- https://lua.expert/ [МОДИФИЦИРОВАННЫЙ С ВЫВОДАМИ]
print("[System] PlayersDamageRequestService инициализирован.")
...
-- Быстрая отправка на сервер без задержек
destroyAsync.Event:Connect(function(...)
	local t = { ... }
	if type(t[1]) == "table" then
		t[1].Reciept = stringrandom(10)
		print("[Event] Отправка таблицы разрушения на сервер. Reciept: " .. t[1].Reciept)
	...
	print("[Event] Отправка запроса урона на сервер типа: " .. tostring(t[1]) .. " | Reciept: " .. t2.Reciept)
...
t.gui.amount.Value:GetPropertyChangedSignal("Value"):Connect(function()
	local v1 = comma_value
	t.gui.amount.Text = v1(math.ceil(t.gui.amount.Value.Value) .. " Voxels")
end)

RequestDestroy.OnClientEvent:Connect(function(p1, p2, p3)
	if p1 == "processed" then return end
	if p1 ~= "amount" and p1 ~= "amount_special" then return end
	if not t.showVoxelBrokeGui.Value then return end

	local function closeMiniEarnedGui(p1)
		if not p1:GetAttribute("closing") and p1:IsA("TextLabel") then
			p1:SetAttribute("closing", true)
			-- Моментальное исчезновение (0.05 сек вместо 0.4)
			TweenService:Create(p1.UIScale, TweenInfo.new(0.05), {Scale = 0}):Play()
			TweenService:Create(p1, TweenInfo.new(0.05), {TextTransparency = 1}):Play()
	...
	if p1 == "amount_special" then
		print("[UI] Получен специальный тип вокселей (amount_special)")
		...
		for v1, v2 in p2 do
			...
			v6.Value.Value = v2
			v6.Parent = extra
			-- Моментальное появление (0.05 сек вместо 0.5)
			TweenService:Create(v6.UIScale, TweenInfo.new(0.05, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Scale = 1}):Play()
		end
		p2 = 0
	elseif p2 <= 0 then
		return
	...
	print("[UI] Добавление обычных разрушенных вокселей: +" .. tostring(p2))

	if not t.isShown then
		-- Ускоренное появление UI (0.05 сек вместо 0.2)
		TweenService:Create(t.gui.amount, TweenInfo.new(0.05), {TextTransparency = 0, TextStrokeTransparency = 0}):Play()
		t.addition = p2
		t.isShown = 35
		t.gui.amount.Value.Value = 0
		-- Быстрое нарастание цифр (0.1 сек вместо 1.0)
		TweenService:Create(t.gui.amount.Value, TweenInfo.new(0.1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Value = t.addition}):Play()
		
		spawn(function()
			repeat
				if not task.wait(0.05) then
					TweenService:Create(t.gui.amount, TweenInfo.new(0.05), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
					TweenService:Create(t.gui.add, TweenInfo.new(0.05), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
					t.isShown = nil
					return
				end
	...
	TweenService:Create(gui.add, TweenInfo.new(0.05, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, 20)}):Play()
	gui.add.Text = "+" .. comma_value((math.ceil(p2)))
	TweenService:Create(gui.add, TweenInfo.new(0.05), {TextTransparency = 0, TextStrokeTransparency = 0}):Play()
	TweenService:Create(gui.amount.Value, TweenInfo.new(0.1, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Value = t.addition}):Play()
end)
...
local function updateCheck()
	...
	for v2, v3 in t2 do
		...
		if v3[2] < v12 then
			v3[1]:Destroy()
			table.remove(t2, v2)
			print("[Garbage] Удален устаревший плейсхолдер блока.")
			continue
		end
		...
		if v3[3] / 4 <= sum then
			v4:Destroy()
			table.remove(t2, v2)
			print("[Garbage] Плейсхолдер блока уничтожен по условию объема.")
		end
	end
end

GlobalParts.ChildRemoved:Connect(function(p1)
	...
	print("[Physics] Создан локальный плейсхолдер для удаленного блока: " .. p1.Name)
end)

-- Кадровый апдейт (вместо task.wait(0.1)) гарантирует отсутствие микрофризов блоков
while true do
	updateCheck()
	task.wait()
end
